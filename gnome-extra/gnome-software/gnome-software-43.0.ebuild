...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="flatpak +firmware gnome gtk-doc sysprof udev +webapps hardcoded-foss-webapps hardcoded-proprietary-webapps"
REQUIRED_USE="
	hardcoded-foss-webapps? ( webapps )
	hardcoded-proprietary-webapps? ( webapps )
"
RESTRICT="!test? ( test )" # See TODO below, TODO needs glib-testing-0, which I can not find.
...
src_configure() {
	local emesonargs=(
		...
		$(meson_use flatpak)
		-Dmalcontent=false # not packaged
		-Drpm_ostree=false
		...
		-Dsoup2=true
		$(meson_use webapps)
		$(meson_use hardcoded-proprietary-webapps hardcoded_proprietary_webapps)
		$(meson_use hardcoded-foss-webapps hardcoded_foss_webapps)
	)
	meson_src_configure
}
